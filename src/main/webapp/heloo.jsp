...
<body>
    ...
    <a href="${empaddress}"> MEMBER EXPENSE TRACKER </a>
    <a href="${IncomeView}">INOCME DETAILS</a>
		</div>
	<!-- /container -->
	<script
	...
	<form action="/addCategories" method="post">
		<tr>
			<td><h3 class="sansserif">Add Category :</h3></td>
			<td><input type="text" name="addcategory" /> <input
	...
	<form action="/deleteCategories" method="post">
		<tr>
			<td><h3 class="sansserif">Delete Category :</h3></td>
			<td><select name="deleteCategory">
					<c:forEach items="${categories}" var="databaseValue">
						<option value="${databaseValue}">${databaseValue}</option>
					</c:forEach>
			</select><input type="submit" value="Delete Category" /></td>
		</tr>
	</form>
</body>
</html>
